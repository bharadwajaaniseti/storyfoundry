...
export default function ChapterComments({ chapterId, className = '' }: ChapterCommentsProps) {
  ...
  const loadComments = async () => {
    try {
      ...
      // Skip API call if chapterId is not a valid UUID or if it's a test ID
      if (!chapterId || chapterId.startsWith('test') || chapterId.length < 10) {
        console.log('Skipping API call for test/invalid chapter ID')
        setComments([])
        return
      }
  ...
  const loadReviews = async () => {
    try {
      ...
      const response = await fetch(`/api/chapters/${chapterId}/reviews`)
