...
function createCurvedArrowPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy}
          Q ${cx - quarterSize} ${cy - quarterSize} ${cx} ${cy - quarterSize}
          Q ${cx + quarterSize} ${cy - quarterSize} ${cx + halfSize} ${cy}
          L ${cx + quarterSize} ${cy + quarterSize}
          L ${cx + halfSize} ${cy + halfSize}
          L ${cx} ${cy}
          L ${cx + halfSize} ${cy - halfSize}
          L ${cx + quarterSize} ${cy - quarterSize} Z`
}

function createMountainPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy + quarterSize}
          L ${cx - quarterSize} ${cy - quarterSize}
          L ${cx - quarterSize/2} ${cy - quarterSize/2}
          L ${cx} ${cy - halfSize}
          L ${cx + quarterSize/2} ${cy - quarterSize/2}
          L ${cx + quarterSize} ${cy - quarterSize}
          L ${cx + halfSize} ${cy + quarterSize} Z`
...
function createForestGroup(cx: number, cy: number, size: number, baseProps: any) {
  const treeSize = size / 3
  const trees = []
  for (let i = 0; i < 3; i++) {
    const x = cx + (i - 1) * treeSize * 0.7
    const y = cy + (i % 2 === 0 ? 0 : treeSize * 0.3)
    const treePath = `M ${x} ${y + treeSize/2}
                     L ${x - treeSize/3} ${y - treeSize/2}
                     L ${x + treeSize/3} ${y - treeSize/2} Z`
    trees.push(<path key={i} d={treePath} {...baseProps} />)
  }
  return <g>{trees}</g>
}

function createWaterPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const amplitude = size / 8
  return `M ${cx - halfSize} ${cy}
          Q ${cx - halfSize/2} ${cy - amplitude} ${cx} ${cy}
          Q ${cx + halfSize/2} ${cy + amplitude} ${cx + halfSize} ${cy}
          L ${cx + halfSize} ${cy + amplitude}
          Q ${cx + halfSize/2} ${cy + amplitude*2} ${cx} ${cy + amplitude}
          Q ${cx - halfSize/2} ${cy} ${cx - halfSize} ${cy + amplitude} Z`
}

function createBuildingPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy + halfSize}
          L ${cx - halfSize} ${cy - quarterSize}
          L ${cx - quarterSize} ${cy - quarterSize}
          L ${cx - quarterSize} ${cy - halfSize}
          L ${cx + quarterSize} ${cy - halfSize}
          L ${cx + quarterSize} ${cy - quarterSize}
          L ${cx + halfSize} ${cy - quarterSize}
          L ${cx + halfSize} ${cy + halfSize} Z`
}

function createTowerPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - quarterSize} ${cy + halfSize}
          L ${cx - quarterSize} ${cy - halfSize}
          L ${cx - halfSize} ${cy - halfSize}
          L ${cx - quarterSize/2} ${cy - halfSize - quarterSize}
          L ${cx + quarterSize/2} ${cy - halfSize - quarterSize}
          L ${cx + halfSize} ${cy - halfSize}
          L ${cx + quarterSize} ${cy - halfSize}
          L ${cx + quarterSize} ${cy + halfSize} Z`
}

function createGatePath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy + halfSize}
          L ${cx - halfSize} ${cy - quarterSize}
          ...
          L ${cx + halfSize} ${cy - quarterSize}
          L ${cx + halfSize} ${cy + halfSize}
          M ${cx - quarterSize/2} ${cy + halfSize}
          L ${cx - quarterSize/2} ${cy}
          L ${cx + quarterSize/2} ${cy}
          L ${cx + quarterSize/2} ${cy + halfSize}`
}

function createBridgePath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy}
          Q ${cx} ${cy - quarterSize} ${cx + halfSize} ${cy}
          L ${cx + halfSize} ${cy + quarterSize/2}
          Q ${cx} ${cy + quarterSize/2 - quarterSize} ${cx - halfSize} ${cy + quarterSize/2} Z`
}

function createRoadPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy - quarterSize/2}
          L ${cx + halfSize} ${cy - quarterSize/2}
          L ${cx + halfSize} ${cy + quarterSize/2}
          L ${cx - halfSize} ${cy + quarterSize/2} Z`
}

function createCompassPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  const paths = []
  
  // Compass rose
  paths.push(`M ${cx} ${cy - halfSize} L ${cx} ${cy + halfSize} M ${cx - halfSize} ${cy} L ${cx + halfSize} ${cy}`)
  paths.push(`M ${cx - quarterSize} ${cy - quarterSize} L ${cx + quarterSize} ${cy + quarterSize}`)
  paths.push(`M ${cx + quarterSize} ${cy - quarterSize} L ${cx - quarterSize} ${cy + quarterSize}`)
  
  return paths
}

function createScrollPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy - quarterSize}
          Q ${cx - halfSize - quarterSize/2} ${cy - quarterSize} ${cx - halfSize - quarterSize/2} ${cy}
          Q ${cx - halfSize - quarterSize/2} ${cy + quarterSize} ${cx - halfSize} ${cy + quarterSize}
          L ${cx + halfSize} ${cy + quarterSize}
          Q ${cx + halfSize + quarterSize/2} ${cy + quarterSize} ${cx + halfSize + quarterSize/2} ${cy}
          Q ${cx + halfSize + quarterSize/2} ${cy - quarterSize} ${cx + halfSize} ${cy - quarterSize} Z`
}

function createFlamePath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx} ${cy + halfSize}
          Q ${cx - quarterSize} ${cy + quarterSize} ${cx - quarterSize/2} ${cy}
          Q ${cx - quarterSize} ${cy - quarterSize} ${cx} ${cy - halfSize}
          Q ${cx + quarterSize} ${cy - quarterSize} ${cx + quarterSize/2} ${cy}
          Q ${cx + quarterSize} ${cy + quarterSize} ${cx} ${cy + halfSize} Z`
}

function createCrystalPath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx} ${cy - halfSize}
          L ${cx - quarterSize} ${cy - quarterSize}
...
function createRunePath(cx: number, cy: number, size: number) {
  const halfSize = size / 2
  const quarterSize = size / 4
  return `M ${cx - halfSize} ${cy - halfSize}
          L ${cx + halfSize} ${cy - halfSize}
          L ${cx + halfSize} ${cy + halfSize}
          L ${cx - halfSize} ${cy + halfSize} Z
          M ${cx - quarterSize} ${cy - quarterSize}
          L ${cx + quarterSize} ${cy + quarterSize}
          M ${cx + quarterSize} ${cy - quarterSize}
          L ${cx - quarterSize} ${cy + quarterSize}
          M ${cx} ${cy - halfSize}
...
function StaticColorPicker({ 
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      {/* Dropdown Panel */}
      {isDropdownOpen && (
        <div className="absolute top-full left-0 mt-2 bg-white border border-gray-200 rounded-lg shadow-lg z-50 min-w-[320px] max-h-[500px] overflow-y-auto">
