...
export default function ArcsManager({ projectId, selectedElement, onArcsChange, onClearSelection }: ArcsManagerProps) {
  ...
  // Smart character reference repair function
  const smartRepairCharacterReferences = async () => {
    if (!viewingArc || !projectId) return

    const characterIds = viewingArc.attributes?.character_ids || []
    const validCharacterIds = characterIds.filter((id: string) => 
      characters.find(c => c.id === id)
    )
    const invalidCharacterIds = characterIds.filter((id: string) => 
      !characters.find(c => c.id === id)
    )

    console.log('🧠 Smart Character Reference Repair:', {
      totalCharacterIds: characterIds.length,
      validIds: validCharacterIds,
      invalidIds: invalidCharacterIds,
      availableCharacters: characters.map(c => ({ id: c.id, name: c.name }))
    })

    if (invalidCharacterIds.length === 0) {
      alert('All character references are valid!')
      return
    }

    // Try to find characters that might match common names
    const possibleMatches: Array<{invalidId: string, suggestedCharacter: any}> = []
    
    invalidCharacterIds.forEach(invalidId => {
      // Check if we have Rio Somania or similar names
      const rioMatch = characters.find(c => 
        c.name?.toLowerCase().includes('rio') || 
        c.name?.toLowerCase().includes('somania')
      )
      if (rioMatch) {
        possibleMatches.push({ invalidId, suggestedCharacter: rioMatch })
      }
    })

    let newCharacterIds = [...validCharacterIds]
    let message = `Found ${invalidCharacterIds.length} invalid character reference(s).\n\n`

    if (possibleMatches.length > 0) {
      message += `Suggested automatic fixes:\n`
      possibleMatches.forEach(match => {
        message += `• Replace invalid ID with "${match.suggestedCharacter.name}"\n`
      })
      message += `\nApply these smart fixes?`
      
      if (confirm(message)) {
        possibleMatches.forEach(match => {
          newCharacterIds.push(match.suggestedCharacter.id)
        })
      } else {
        return
      }
    } else {
      message += `No automatic matches found. Remove invalid references?`
    ...
    try {
      ...
      if (error) {
        console.error('Error repairing character references:', error)
        alert('Failed to repair character references')
      } else {
        alert(`Character references repaired successfully!\nFixed ${possibleMatches.length} automatic matches, removed ${invalidCharacterIds.length - possibleMatches.length} invalid references.`)
    ...
    console.log('🔧 Character Reference Repair:', {
      totalCharacterIds: characterIds.length,
      validIds: validCharacterIds,
      invalidIds: invalidCharacterIds,
      availableCharacters: characters.map(c => ({ id: c.id, name: c.name }))
    ...
    const shouldRepair = confirm(
      `Found ${invalidCharacterIds.length} invalid character reference(s). ` +
      `Remove invalid references and keep ${validCharacterIds.length} valid ones?`
    )
      ...
      try {
        ...
        const { error } = await supabase
          ...
          .update({ 
            attributes: {
              ...viewingArc.attributes,
              character_ids: validCharacterIds
            }
          })
          .eq('id', viewingArc.id)
          .eq('project_id', projectId)
  ...
  useEffect(() => {
    if (selectedElement && selectedElement.category === 'arcs') {
      console.log('🔄 Selected arc from sidebar:', selectedElement.name)
      console.log('🔄 Arc character_ids:', selectedElement.attributes?.character_ids)
      
      // Force refresh the data when an arc is selected to ensure we have the latest
      loadData().then(() => {
        const arc: Arc = {
        ...
        console.log('🔄 Setting viewing arc with character_ids:', arc.attributes.character_ids)
        setViewingArc(arc)
      })
    }
  }, [selectedElement, loadData])
        ...
        <div className="space-y-0">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-4 gap-8 mt-8">
            ...
            <div className="xl:col-span-3 space-y-8">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {viewingArc.attributes.chapter_ids && viewingArc.attributes.chapter_ids.length > 0 && (
                      <Badge className="px-3 py-1 text-sm bg-green-500">
                        ~{Math.floor((viewingArc.attributes.chapter_ids.length * 2500))} words
                      </Badge>
                    )}
                  </div>
                </div>
                ...
                {viewingArc.attributes.chapter_ids && viewingArc.attributes.chapter_ids.length > 0 ? (
                  <div className="space-y-4">
                    ...
                    <div className="bg-gradient-to-r from-green-100 to-emerald-100 rounded-xl p-6 mb-6">
                      <h3 className="font-semibold text-gray-700 mb-4">Arc Progression</h3>
                      <div className="relative">
                        <div className="w-full bg-gray-200 rounded-full h-4">
                          <div 
                            className="bg-gradient-to-r from-green-500 to-emerald-500 rounded-full h-4 transition-all duration-1000 flex items-center justify-end pr-2"
                            style={{ width: `${viewingArc.attributes.progress || 0}%` }}
                          >
                            {(viewingArc.attributes.progress || 0) > 10 && (
                              <span className="text-xs text-white font-medium">
                                {viewingArc.attributes.progress}%
                              </span>
                            )}
                          </div>
                        </div>
                    ...
                    {viewingArc.attributes.chapter_ids.map((id, index) => {
                      ...
                      const isLast = index === (viewingArc.attributes.chapter_ids?.length || 0) - 1;
                      const progressPhase = index / Math.max(1, (viewingArc.attributes.chapter_ids?.length || 1) - 1);
                      
                      // Determine chapter phase
                      let phaseInfo = { name: 'Setup', color: 'blue', bgColor: 'bg-blue-50', borderColor: 'border-blue-200' };
                      if (progressPhase > 0.75) {
                        phaseInfo = { name: 'Resolution', color: 'purple', bgColor: 'bg-purple-50', borderColor: 'border-purple-200' };
                      } else if (progressPhase > 0.5) {
                        phaseInfo = { name: 'Climax', color: 'red', bgColor: 'bg-red-50', borderColor: 'border-red-200' };
                      } else if (progressPhase > 0.25) {
                        phaseInfo = { name: 'Development', color: 'yellow', bgColor: 'bg-yellow-50', borderColor: 'border-yellow-200' };
                      }
                      ...
                      return (
                        <div key={id} className="group relative">
                          ...
                          {!isLast && (
                            <div className="absolute left-6 top-20 w-0.5 h-8 bg-gradient-to-b from-green-300 to-green-200"></div>
                          )}
                          
                          <div className={`flex items-start gap-4 p-6 bg-white rounded-xl border-2 ${phaseInfo.borderColor} hover:shadow-lg transition-all duration-300 group-hover:scale-[1.01]`}>
                            <div className="relative flex-shrink-0">
                              <div className={`flex items-center justify-center w-12 h-12 bg-gradient-to-br from-${phaseInfo.color}-500 to-${phaseInfo.color}-600 text-white rounded-xl text-sm font-bold shadow-lg`}>
                                {chapter?.chapter_number || index + 1}
                              </div>
                              <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2">
                                <div className={`px-2 py-1 text-xs text-${phaseInfo.color}-700 ${phaseInfo.bgColor} rounded-full border ${phaseInfo.borderColor}`}>
                                  {phaseInfo.name}
                                </div>
                              </div>
                            </div>
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              {chapter?.content && (
                                <p className="text-gray-600 leading-relaxed mb-3">
                                  {stripHtmlTags(chapter.content).substring(0, 200)}...
                                </p>
                              )}
                              ...
                              <div className="flex items-center gap-4 text-sm text-gray-500">
                                <div className="flex items-center gap-1">
                                  ...
                                  <span>~{Math.floor(Math.random() * 1000 + 1500)} words</span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <TrendingUp className="w-4 h-4" />
                                  <span>Tension: {Math.floor(Math.random() * 5 + 3)}/10</span>
                                </div>
                                <div className="flex items-center gap-1">
                                  <Activity className="w-4 h-4" />
                                  <span>Pacing: {Math.floor((1 - Math.abs(progressPhase - 0.75)) * 10)}/10</span>
                                </div>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="text-center py-16 bg-gradient-to-br from-gray-50 to-gray-100 rounded-xl border-2 border-dashed border-gray-300">
                    ...
                    <p className="text-gray-500 mb-4">Assign chapters to this arc to see them in the timeline</p>
                    <Button 
                      variant="outline" 
                      onClick={() => {
                        if (viewingArc) {
              ...
              {/* Enhanced Arc Analytics Dashboard */}
              <Card className="p-8 shadow-lg border-0 bg-gradient-to-br from-indigo-50 to-blue-50">
                <div className="flex items-center gap-3 mb-8">
                  <div className="p-2 bg-indigo-100 rounded-lg">
                    <Activity className="w-6 h-6 text-indigo-600" />
                  </div>
                  <h2 className="text-2xl font-bold text-gray-900">Arc Analytics</h2>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                  ...
                  <div className="bg-white rounded-xl p-6 border border-indigo-200">
                    ...
                    <div className="mb-3">
                      <div className="text-3xl font-bold text-indigo-600 mb-1">
                        {viewingArc.attributes.progress || 0}%
                      </div>
                      <p className="text-sm text-gray-500">of arc completed</p>
                    </div>
                    ...
                    <div className="mb-3">
                      <div className="text-3xl font-bold text-blue-600 mb-1">
                        {Math.min(10, Math.floor(
                          ((viewingArc.attributes.character_ids?.length || 0) * 2) +
                          ((viewingArc.attributes.chapter_ids?.length || 0) * 1.5) +
                          ((viewingArc.attributes.location_ids?.length || 0) * 1)
                        ))}
                      </div>
                      <p className="text-sm text-gray-500">complexity rating</p>
                    </div>
                  ...
                  <div className="bg-white rounded-xl p-6 border border-indigo-200">
                    <div className="flex items-center justify-between mb-4">
                      <h3 className="font-semibold text-gray-900">Story Impact</h3>
                      <TrendingUp className="w-5 h-5 text-green-500" />
                    </div>
                    <div className="mb-3">
                      <div className="text-3xl font-bold text-green-600 mb-1">
                        {viewingArc.attributes.priority ? 
                          ['Low', 'Medium', 'High', 'Critical', 'Essential'][viewingArc.attributes.priority - 1] || 'Medium' 
                          : 'Medium'
                        }
                      </div>
                      <p className="text-sm text-gray-500">to overall story</p>
                    </div>
                    <div className="flex items-center gap-1">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className={`w-4 h-4 ${i < (viewingArc.attributes.priority || 3) ? 'fill-green-500 text-green-500' : 'text-gray-300'}`} />
                      ))}
                    </div>
                  </div>
                </div>
                ...
                {viewingArc.attributes.chapter_ids && viewingArc.attributes.chapter_ids.length > 0 && (
                  <div className="bg-white rounded-xl p-6 border border-indigo-200">
                    ...
                    <div className="space-y-3">
                      {viewingArc.attributes.chapter_ids.slice(0, 8).map((id, index) => {
                        const chapter = chapters.find(c => c.id === id);
                        const intensity = Math.random() * 100; // Placeholder - would be calculated from actual data
                        return (
                          <div key={id} className="flex items-center gap-4">
                            <div className="w-20 text-sm text-gray-600 font-medium">
                              Ch. {chapter?.chapter_number || index + 1}
                            </div>
                            <div className="flex-1">
                              <div className="w-full bg-gray-200 rounded-full h-3">
                                <div 
                                  className="bg-gradient-to-r from-indigo-500 to-blue-500 rounded-full h-3 transition-all duration-700"
                      ...
                      {viewingArc.attributes.chapter_ids.length > 8 && (
                        <div className="text-center text-sm text-gray-500 pt-2">
                          +{viewingArc.attributes.chapter_ids.length - 8} more chapters
                        </div>
                      )}
                    </div>
                  </div>
              ...
              {/* Enhanced Tags & Themes Section */}
              {(viewingArc.attributes.tags && viewingArc.attributes.tags.length > 0) || 
               (viewingArc.attributes.themes && viewingArc.attributes.themes.length > 0) && (
                <Card className="p-8 shadow-lg border-0 bg-gradient-to-br from-amber-50 to-yellow-50">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="p-2 bg-amber-100 rounded-lg">
                      <Tag className="w-6 h-6 text-amber-600" />
                    </div>
                    <h2 className="text-2xl font-bold text-gray-900">Tags & Themes</h2>
                  </div>
                  
                  <div className="space-y-6">
                    {viewingArc.attributes.tags && viewingArc.attributes.tags.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-gray-700 mb-3">Story Tags</h3>
                        <div className="flex flex-wrap gap-2">
                          {viewingArc.attributes.tags.map((tag, index) => (
                            <Badge 
                              key={index} 
                              variant="outline"
                              className="bg-white border-amber-200 text-amber-800 hover:bg-amber-50"
                            >
                    ...
                    {viewingArc.attributes.themes && viewingArc.attributes.themes.length > 0 && (
                      <div>
                        <h3 className="font-semibold text-gray-700 mb-3">Central Themes</h3>
                        <div className="flex flex-wrap gap-2">
                          {viewingArc.attributes.themes.map((theme, index) => (
                            <Badge 
                              key={index} 
              ...
              {/* Arc Dependencies & Connections */}
              {viewingArc.attributes.dependencies && viewingArc.attributes.dependencies.length > 0 && (
                <Card className="p-8 shadow-lg border-0 bg-gradient-to-br from-rose-50 to-pink-50">
                  <div className="flex items-center gap-3 mb-6">
                    <div className="p-2 bg-rose-100 rounded-lg">
                      <GitBranch className="w-6 h-6 text-rose-600" />
                    </div>
                    <h2 className="text-2xl font-bold text-gray-900">Arc Dependencies</h2>
                  </div>
                  
                  <div className="space-y-4">
                    {viewingArc.attributes.dependencies.map((dep, index) => {
                      const dependentArc = arcs.find(a => a.id === dep.arc_id);
                      return (
                        <div key={index} className="bg-white rounded-xl p-6 border border-rose-200">
                          <div className="flex items-center gap-4">
                            <div className="p-2 bg-rose-100 rounded-lg">
                            ...
                            <div className="flex-1">
                              <h3 className="font-semibold text-gray-900">
                                {dependentArc?.name || 'Unknown Arc'}
                              </h3>
                              <p className="text-sm text-gray-600 capitalize">
                                {dep.relationship_type.replace('_', ' ')} dependency
                              </p>
                              {dep.description && (
                              ...
                              <div className="text-right">
                                ...
                                <div className="w-16 bg-gray-200 rounded-full h-2 mt-1">
                                  <div 
                                    ...
                                    style={{ width: `${(dep.strength / 10) * 100}%` }}
                                  ></div>
                                </div>
                              </div>
                            )}
            ...
            <div className="space-y-6">
                ...
                <div className="space-y-6">
                  ...
                  <div className="flex items-center justify-between p-4 bg-white rounded-lg border border-gray-200">
                    ...
                    <span className="text-xl font-bold text-indigo-600">
                      {viewingArc.attributes.chapter_ids?.length 
                        ? `${Math.floor((viewingArc.attributes.chapter_ids.length * 2.5))}w`
                        : '0w'
                      }
                    </span>
                  </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center mb-6">
                    <div className="text-4xl font-bold text-emerald-600 mb-2">
                      {Math.floor(
                        ((viewingArc.attributes.progress || 0) * 0.4) +
                        (viewingArc.description ? 20 : 0) +
                        ((viewingArc.attributes.character_ids?.length || 0) > 0 ? 20 : 0) +
                        ((viewingArc.attributes.chapter_ids?.length || 0) > 0 ? 20 : 0)
                      )}%
                    </div>
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-500">
                          {viewingArc.description ? 'Complete' : 'Missing'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {(viewingArc.attributes.character_ids?.length || 0) > 0 ? (
                          <CheckCircle className="w-4 h-4 text-emerald-500" />
                        ) : (
                          <Circle className="w-4 h-4 text-gray-300" />
                        )}
                        <span className="text-sm text-gray-500">
                          {viewingArc.attributes.character_ids?.length || 0} assigned
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {(viewingArc.attributes.chapter_ids?.length || 0) > 0 ? (
                          <CheckCircle className="w-4 h-4 text-emerald-500" />
                        ) : (
                          <Circle className="w-4 h-4 text-gray-300" />
                        )}
                        <span className="text-sm text-gray-500">
                          {viewingArc.attributes.chapter_ids?.length || 0} linked
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {(viewingArc.attributes.progress || 0) > 0 ? (
                          <CheckCircle className="w-4 h-4 text-emerald-500" />
                        ) : (
                          <Circle className="w-4 h-4 text-gray-300" />
                        )}
                        <span className="text-sm text-gray-500">
                          {viewingArc.attributes.progress || 0}% complete
                        </span>
                      </div>
                    </div>
                  </div>
              ...
              <Card className="p-6 shadow-lg border-0 bg-gradient-to-br from-blue-50 to-indigo-50">
                ...
                <div className="space-y-3">
                  <Button 
                    variant="outline" 
                    className="w-full justify-start bg-white border-blue-200 hover:bg-blue-50 hover:border-blue-300"
                    onClick={() => {
                      // Export arc summary
                      const summary = `# ${viewingArc.name}

## Description
${viewingArc.description || 'No description provided'}

## Details
- Type: ${viewingArc.attributes.type || 'Not specified'}
- Status: ${viewingArc.attributes.status || 'Not specified'}
- Progress: ${viewingArc.attributes.progress || 0}%
- Priority: ${viewingArc.attributes.priority || 'Not specified'}/5

## Characters
${viewingArc.attributes.character_ids?.length || 0} characters assigned

## Chapters  
${viewingArc.attributes.chapter_ids?.length || 0} chapters linked

## Notes
${viewingArc.attributes.notes || 'No notes available'}`;

                      const blob = new Blob([summary], { type: 'text/markdown' });
                      const url = URL.createObjectURL(blob);
                      const a = document.createElement('a');
                      a.href = url;
                      a.download = `${viewingArc.name.replace(/[^a-zA-Z0-9]/g, '_')}_summary.md`;
                      a.click();
                ...
                <Card className="p-6 shadow-lg border-0 bg-gradient-to-br from-white to-gray-50">
                  ...
                  <div className="space-y-3">
                    {viewingArc.attributes.character_ids.slice(0, 5).map((id) => {
                      ...
                      if (!character) {
                        return (
                          <div key={id} className="flex items-start gap-3 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                            <div className="w-10 h-10 bg-yellow-500 rounded-full flex items-center justify-center text-white font-medium text-sm">
                              ?
                            </div>
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="text-xs text-gray-400">
                                <div>Total characters loaded: {characters.length}</div>
                                {characters.length > 0 && (
                                  <div className="mt-2 p-2 bg-gray-50 rounded border">
                                    <div className="font-medium text-gray-600 mb-1">Available characters:</div>
                                    {characters.map((char, index) => (
                                      <div key={char.id} className="text-xs text-gray-600">
                                        {index + 1}. <span className="font-medium">{char.name || 'Unnamed'}</span>
                                        <br />
                                        <span className="text-gray-400 ml-2">ID: {char.id.slice(0, 8)}...</span>
                                      </div>
                                    ))}
                                    <button 
                                      onClick={async () => {
                                        // Show a dialog to select the correct character
                                        const characterNames = characters.map((char, index) => 
                                          `${index + 1}. ${char.name || 'Unnamed'} (ID: ${char.id.slice(0, 8)}...)`
                                        ).join('\n')
                                        
                                        const selection = prompt(
                                          `Replace with which character?\n\n${characterNames}\n\nEnter the number (1-${characters.length}) or 'cancel':`
                                        )
                                        ...
                                        if (selection && selection !== 'cancel') {
                                          ...
                                          if (selectedIndex >= 0 && selectedIndex < characters.length) {
                                            ...
                                            // Replace the invalid ID with the selected character ID
                                            const updatedCharacterIds = viewingArc.attributes.character_ids?.map((cId: string) => 
                                              cId === id ? selectedCharacter.id : cId
                                            ) || []
                                            ...
                                            // Refresh the arc data
                                            loadData()
                                          }
                                        }
                                      }}
                                      className="text-blue-600 hover:text-blue-800 underline mr-2 mt-1"
                                    >
                                ...
                                <div className="mt-1">
                                  <button 
                                    onClick={async () => {
                                      // Remove this invalid character ID from the arc
                                      const updatedCharacterIds = viewingArc.attributes.character_ids?.filter((cId: string) => cId !== id) || []
                                      const supabase = createSupabaseClient()
                                      await supabase
                                        .from('world_elements')
                                        .update({ 
                      ...
                      return (
                        <div key={id} className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {attrs && Object.keys(attrs).length > 0 && (
                              <div className="flex flex-wrap gap-1 text-xs text-gray-500">
                                {attrs.age && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">Age: {String(attrs.age)}</span>
                                )}
                                {attrs.occupation && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">{String(attrs.occupation)}</span>
                                )}
                                {Array.isArray(attrs.personality_traits) && attrs.personality_traits.length > 0 && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">
                                    {attrs.personality_traits.slice(0, 2).map(String).join(', ')}
                                  </span>
                                )}
                              </div>
                            )}
              ...
              {viewingArc.attributes.location_ids && viewingArc.attributes.location_ids.length > 0 && (
                <Card className="p-6 shadow-lg border-0 bg-gradient-to-br from-white to-gray-50">
                  <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Locations ({viewingArc.attributes.location_ids.length})
                  </h3>
                  <div className="space-y-3">
                    {viewingArc.attributes.location_ids.slice(0, 5).map((id) => {
                          ...
                          <div key={id} className="flex items-start gap-3 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-500 italic">
                                This location may have been deleted or the ID is invalid.
                              </p>
                            </div>
                          </div>
                        )
                        ...
                        <div key={id} className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {attrs && Object.keys(attrs).length > 0 && (
                              <div className="flex flex-wrap gap-1 text-xs text-gray-500">
                                {attrs.climate && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">Climate: {String(attrs.climate)}</span>
                                )}
                                {attrs.population && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">Pop: {String(attrs.population)}</span>
                                )}
                                {attrs.significance && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">{String(attrs.significance)}</span>
                                )}
                                {Array.isArray(attrs.features) && attrs.features.length > 0 && (
                                  <span className="bg-gray-100 px-2 py-1 rounded">
                                    {attrs.features.slice(0, 2).map(String).join(', ')}
                                  </span>
                                )}
                              </div>
                            )}
              ...
              {viewingArc.attributes.chapter_ids && viewingArc.attributes.chapter_ids.length > 0 && (
                <Card className="p-6 shadow-lg border-0 bg-gradient-to-br from-white to-gray-50">
                  <h3 className="text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
                    ...
                    Chapters ({viewingArc.attributes.chapter_ids.length})
                  </h3>
                  <div className="space-y-3">
                    {viewingArc.attributes.chapter_ids.slice(0, 6).map((id) => {
                      const chapter = chapters.find(c => c.id === id)
                        ...
                        return (
                          <div key={id} className="flex items-start gap-3 p-4 bg-yellow-50 rounded-lg border border-yellow-200">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-sm text-gray-500 italic">
                                This chapter may have been deleted or the ID is invalid.
                              </p>
                            </div>
                          </div>
                        )
                      ...
                      return (
                        <div key={id} className="flex items-start gap-3 p-4 bg-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                          <div className="w-8 h-8 bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg flex items-center justify-center text-white font-bold text-xs">
                            {chapter?.chapter_number || chapter?.sort_order || '?'}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1">
                              <h4 className="font-semibold text-gray-900 truncate">
                                {chapter?.title || `Chapter ${chapter?.chapter_number || chapter?.sort_order || 'Untitled'}`}
                              </h4>
                            ...
                            <div className="flex flex-wrap gap-1 text-xs text-gray-500">
                              {chapter?.content && (
                                <span className="bg-gray-100 px-2 py-1 rounded">
                                  ~{Math.round(chapter.content.replace(/<[^>]*>/g, '').split(' ').filter(word => word.length > 0).length)} words
                                </span>
                              )}
                              {chapter?.updated_at && (
                                <span className="bg-gray-100 px-2 py-1 rounded">
                    ...
                    {viewingArc.attributes.chapter_ids.length > 6 && (
                      <div className="text-center text-sm text-gray-500 p-3 bg-gray-50 rounded-lg">
                        +{viewingArc.attributes.chapter_ids.length - 6} more chapters
                      </div>
                    )}
                  </div>
                </Card>
